...
public class SkillSlot : MonoBehaviour
{
    ...
    public void Init()
    {
        ...
        var slotObject = new GameObject("Skill_Slot");          // 생성할 스킬을 담아둘 오브젝트 생성
        slotObject.transform.SetParent(transform, false);       // 부모 설정

        foreach (var buttonData in buttonDatas)                         // 스킬 버튼 데이터 List 순회
        {
            var excutor = Instantiate(excutorPrefab);                   // 스킬 실행기 생성
            excutor.transform.SetParent(slotObject.transform, false);   // 실행기 오브젝트의 부모 설정
